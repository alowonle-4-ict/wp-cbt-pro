 ...
 * @var string $action
 * @var \WPCBTPro\Questions\Types\Programming\ProgrammingAdminEditor $editor
 */
if (!defined('ABSPATH')) {
    exit;
}

$listUrl = add_query_arg(['page' => 'wpcbtpro-programming'], admin_url('admin.php'));
?>
<div class="wrap wpcbtpro-wrap">
    <h1><?php echo $action === 'edit' ? esc_html__('Edit Programming Question', 'wp-cbt-pro') : esc_html__('Add Programming Question', 'wp-cbt-pro'); ?></h1>
    ...
    <form method="post" class="wpcbtpro-form">
        <?php wp_nonce_field('wpcbtpro_save_programming', 'wpcbtpro_programming_nonce'); ?>
        <input type="hidden" name="question_id" value="<?php echo esc_attr((string) ($question['id'] ?? 0)); ?>">

        <table class="form-table" role="presentation">
            <tr>
                <th><label for="content"><?php esc_html_e('Problem statement', 'wp-cbt-pro'); ?> <span class="required">*</span></label></th>
                <td><textarea id="content" name="content" rows="6" class="large-text"><?php echo esc_textarea($question['content'] ?? ''); ?></textarea></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td><input type="number" min="0.5" step="0.5" id="marks" name="marks" value="<?php echo esc_attr((string) ($question['marks'] ?? 10)); ?>"></td>
            </tr>
