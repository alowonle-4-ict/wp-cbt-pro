...
<div class="wrap wpcbtpro-wrap">
    <h1><?php esc_html_e('Assignment Preview', 'wp-cbt-pro'); ?></h1>
    <p><?php esc_html_e('Review each detected row below. Rows with errors are unchecked and cannot be assigned — fix them in the spreadsheet and re-upload. Only checked rows are assigned.', 'wp-cbt-pro'); ?></p>

    <form method="post">
        <?php wp_nonce_field('wpcbtpro_exam_assignment_confirm_' . $session, 'wpcbtpro_exam_assignment_confirm_nonce'); ?>
            ...
            <tbody>
                <?php foreach ($rows as $index => $row): $input = $row['input']; ?>
                    <tr>
                        ...
                        <td><?php echo esc_html($row['exam'] !== null ? $row['exam']['name'] : $input['exam']); ?></td>
        ...
        <?php submit_button(__('Assign Selected Candidates', 'wp-cbt-pro')); ?>
    </form>
</div>
