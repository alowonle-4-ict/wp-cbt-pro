...
/**
 * Upload -> preview -> confirm, the same shape as CandidateBulkImportAdminController
 * and ExamRosterAdminController — but global rather than scoped to one exam,
 * since each row's own "Exam" column decides where it goes (see
 * ExamAssignmentImportService).
 */
final class ExamAssignmentAdminController
{
    private const TRANSIENT_PREFIX = 'wpcbtpro_exam_assignment_';
    private const FAILURES_TRANSIENT_PREFIX = 'wpcbtpro_exam_assignment_failures_';

    public function __construct(
        private readonly ExamAssignmentImportService $importService,
    ...
    public function maybeProcessRequest(): void
    {
        ...
        if (($_GET['page'] ?? '') !== 'wpcbtpro-exam-assignment') {
            return;
        }

        if (!current_user_can(Capabilities::MANAGE_CBT_EXAMS) || !current_user_can(Capabilities::MANAGE_CBT_CANDIDATES)) {
        ...
        if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && isset($_POST['wpcbtpro_exam_assignment_upload_nonce'])) {
            $this->handleUpload();
            return;
        }

        if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && isset($_POST['wpcbtpro_exam_assignment_confirm_nonce'])) {
            $this->handleConfirm();
        }
    ...
    {
        if (!current_user_can(Capabilities::MANAGE_CBT_EXAMS) || !current_user_can(Capabilities::MANAGE_CBT_CANDIDATES)) {
            wp_die(esc_html__('You do not have permission to assign candidates to exams.', 'wp-cbt-pro'));
        }
    ...
    private function renderUploadForm(): void
    {
        ...
        include WPCBTPRO_PATH . 'admin/views/exam-assignment-upload.php';
    }

    /** @param array<int, array<string, mixed>> $rows */
    private function renderPreview(string $session, array $rows): void
    {
        include WPCBTPRO_PATH . 'admin/views/exam-assignment-preview.php';
    }

    private function handleUpload(): void
    {
        check_admin_referer('wpcbtpro_exam_assignment_upload', 'wpcbtpro_exam_assignment_upload_nonce');

        if (empty($_FILES['wpcbtpro_assignment']['name'])) {
            wp_die(esc_html__('No file was uploaded.', 'wp-cbt-pro'));
        }

        $filename = sanitize_file_name($_FILES['wpcbtpro_assignment']['name']);
        ...
        $tmpPath = $_FILES['wpcbtpro_assignment']['tmp_name'] ?? '';
        ...
        if ($rows === []) {
            wp_die(esc_html__('No candidate rows were found in this file. The first row must be a header with at least "First Name", "Last Name", and "Exam" columns.', 'wp-cbt-pro'));
        }
    ...
    private function handleConfirm(): void
    {
        ...
        check_admin_referer('wpcbtpro_exam_assignment_confirm_' . $session, 'wpcbtpro_exam_assignment_confirm_nonce');
        ...
        foreach ($rows as $index => $row) {
            ...
            $result = $this->importService->importRow($row);
